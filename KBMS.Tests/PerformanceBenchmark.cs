...
public class PerformanceBenchmarkV3 : IDisposable
{
    ...
    public void V3_Comprehensive_Performance_Benchmark()
    {
        ...
        string logPath = Path.Combine(Path.GetTempPath(), "storage_v3_results.txt");
    ...
    public void V3_Memory_IO_Tradeoff_Comparison()
    {
        ...
        void RunExperiment(int poolSize, string label)
        {
            ...
            try {
                ...
                var router = new StorageRouter(pool);
        ...
        string reportPath = Path.Combine(Path.GetTempPath(), "buffer_pool_comparison.txt");
    ...
    [Fact]
    public void V3_ManualIndex_Benchmark()
    {
        const int recordCount = 100000;
        const string kbName = "IndexBenchKB";
        const string conceptName = "Patient";
        
        // 1. Concept WITHOUT explicit indexes (only id is auto-indexed)
        var conceptNoIndex = new Concept { 
            Name = conceptName,
            Indexes = new List<ConceptIndex>() 
        };
        
        // 2. Concept WITH explicit index on SSN
        var conceptWithIndex = new Concept {
            Name = conceptName + "_Indexed",
            Indexes = new List<ConceptIndex> {
                new ConceptIndex { Name = "idx_ssn", Fields = new List<string> { "SSN" } }
            }
        };

        var objectsNoIndex = new List<ObjectInstance>();
        var objectsWithIndex = new List<ObjectInstance>();

        for (int i = 0; i < recordCount; i++)
        {
            var ssn = "SSN_" + i.ToString("D7"); // Unique SSN
            
            objectsNoIndex.Add(new ObjectInstance {
                Id = Guid.NewGuid(), ConceptName = conceptNoIndex.Name,
                Values = new Dictionary<string, object> { ["id"] = i, ["SSN"] = ssn, ["Data"] = "PatientData" }
            });
            
            objectsWithIndex.Add(new ObjectInstance {
                Id = Guid.NewGuid(), ConceptName = conceptWithIndex.Name,
                Values = new Dictionary<string, object> { ["id"] = i, ["SSN"] = ssn, ["Data"] = "PatientData" }
            });
        }
        ...
        // --- Benchmark INSERT ---
        var sw = Stopwatch.StartNew();
        _data.BulkInsertObjects(kbName, objectsNoIndex, conceptNoIndex);
        sw.Stop();
        _output.WriteLine($"[Insert] Without Index ({recordCount} records): {sw.ElapsedMilliseconds} ms");

        sw.Restart();
        _data.BulkInsertObjects(kbName, objectsWithIndex, conceptWithIndex);
        sw.Stop();
        _output.WriteLine($"[Insert] With Index (SSN) ({recordCount} records): {sw.ElapsedMilliseconds} ms");

        // --- Benchmark SEARCH ---
        // Simulating the query: FIND Patient WHERE SSN = 'SSN_0050000'
        string targetSSN = "SSN_0050000";
        
        // Scenario 1: Without Index (Full Table Scan)
        sw.Restart();
        var allData = _data.SelectObjects(kbName, conceptNoIndex.Name);
        int noIndexMatches = 0;
        foreach (var obj in allData) {
            if (obj.Values.TryGetValue("SSN", out var val) && val.ToString() == targetSSN) {
                noIndexMatches++;
            }
        }
        sw.Stop();
        _output.WriteLine($"[Search] Without Index (Full Scan): {sw.ElapsedMilliseconds} ms (Found {noIndexMatches})");

        // Scenario 2: With Index (Index Scan O(log N))
        sw.Restart();
        var indexedData = _data.SelectByValue(kbName, conceptWithIndex.Name, "idx_ssn", targetSSN);
        int indexMatches = indexedData.Count;
        sw.Stop();
        _output.WriteLine($"[Search] With Index (Fast Path): {sw.ElapsedMilliseconds} ms (Found {indexMatches})");
        
        Assert.Equal(1, noIndexMatches);
        Assert.Equal(1, indexMatches);
    }
}
