...
using KBMS.Knowledge.Core.Optimizer;
using Tuple = KBMS.Storage.Core.Tuple;

namespace KBMS.Knowledge.Core;
...
public class StorageRouter
{
    ...
    public StorageRouter(StoragePool storagePool)
    {
        _storagePool = storagePool;
        _indexManager = new IndexManager(storagePool);
    }
    ...
    /// </summary>
    public bool InsertObject(string kbName, ObjectInstance obj, Concept? concept = null)
    {
        try
        {
            ...
            lock (_catalogLock)
            {
                ...
                // --- UPDATE VALUE INDEX ---
                if (obj.Values.TryGetValue("id", out var idVal) && idVal != null)
                {
                    _indexManager.InsertIndexEntry(kbName, obj.ConceptName, "id", idVal.ToString()!, new RecordId(page.PageId, slotId));
                }

                if (concept != null && concept.Indexes != null)
                {
                    foreach (var idx in concept.Indexes)
                    {
                        var keyParts = new List<string>();
                        bool allFieldsPresent = true;
                        foreach (var f in idx.Fields)
                        {
                            if (obj.Values.TryGetValue(f, out var val) && val != null)
                                keyParts.Add(val.ToString()!);
                            else
                                allFieldsPresent = false;
                        }
                        if (allFieldsPresent && keyParts.Count > 0)
                        {
                            var compositeKey = string.Join("|", keyParts);
                            _indexManager.InsertIndexEntry(kbName, obj.ConceptName, idx.Name, compositeKey, new RecordId(page.PageId, slotId));
                        }
                    }
                }
                // --------------------
        ...
        {
            Console.Error.WriteLine($"[StorageRouter] InsertObject failed: {ex.Message}");
            return false;
        }
    }
    ...
    /// </summary>
    public int BulkInsertObjects(string kbName, List<ObjectInstance> objects, Concept? concept = null)
    {
        ...
        {
            Console.Error.WriteLine($"[StorageRouter] BulkInsert failed: {ex.Message}");
            return successCount;
        }
    }

    public void BackfillIndex(string kbName, string conceptName, Models.ConceptIndex indexDef)
    {
        var objects = SelectObjects(kbName, conceptName);
        var managers = _storagePool.GetManagers(kbName);
        
        lock (_catalogLock)
        {
            var catalogKey = $"{kbName}:{conceptName}";
            if (!_pageCatalog.ContainsKey(catalogKey)) return;
            var pageIds = _pageCatalog[catalogKey];
            
            foreach (var pageId in pageIds)
            {
                var page = managers.Bpm.FetchPage(pageId);
                if (page == null) continue;
                var sp = new SlottedPage(page);
                for (int slotId = 0; slotId < sp.TupleCount; slotId++)
                {
                    var raw = sp.GetTuple(slotId);
                    if (raw == null) continue;
                    var tuple = Tuple.Deserialize(raw);
                    var obj = TupleToObject(tuple, conceptName, kbName);
                    
                    var keyParts = new List<string>();
                    bool allFieldsPresent = true;
                    foreach (var f in indexDef.Fields)
                    {
                        if (obj.Values.TryGetValue(f, out var val) && val != null)
                            keyParts.Add(val.ToString()!);
                        else
                    ...
                    {
                        var compositeKey = string.Join("|", keyParts);
                        _indexManager.InsertIndexEntry(kbName, conceptName, indexDef.Name, compositeKey, new RecordId(page.PageId, slotId));
                    }
                }
                managers.Bpm.UnpinPage(page.PageId, false);
            }
        }
    }

    public void DropConceptIndex(string kbName, string conceptName, string indexName)
    {
        // To be safe, we might just drop all indices for the KB and rebuild, but IndexManager drop takes KB name currently.
        // We'll rely on the logical metadata removal in Concept.Indexes for now. 
        // V4 can add targeted index dropping in IndexManager.
    }
    ...
    {
        var results = new List<ObjectInstance>();
        var recordId = _indexManager.SearchIndex(kbName, conceptName, fieldName, value);
        
        if (recordId == null) return results;

        var managers = _storagePool.GetManagers(kbName);
        var page = managers.Bpm.FetchPage(recordId.Value.PageId);
        if (page != null)
        {
            var slottedPage = new SlottedPage(page);
            var tupleData = slottedPage.GetTuple(recordId.Value.SlotId);
            if (tupleData != null)
            {
                var tuple = Tuple.Deserialize(tupleData);
                var obj = TupleToObject(tuple, conceptName, kbName);
                if (obj != null) results.Add(obj);
            }
            managers.Bpm.UnpinPage(recordId.Value.PageId, false);
        }
        ...
        foreach (var pageId in pageIds)
        {
            ...
            for (int i = 0; i < sp.TupleCount; i++)
            {
                ...
                if (obj.Id == id)
                {
                    ...
                    return InsertObject(kbName, updated, concept);
                }
            }
    ...
    public bool DropAllMappings(string kbName)
    {
        lock (_catalogLock)
        {
            ...
            // Drop indices conceptually
            // In a real implementation we would drop the B+ tree pages from the buffer pool
            _indexManager.DropIndex(kbName);
