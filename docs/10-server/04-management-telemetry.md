# 10.4. Quản trị & Đo lường từ xa (Telemetry)
...
## 1. Giám sát Hệ thống (Stats & Sessions)
...
2.  **Write to System KB**: Ghi trực tiếp một Fact mới vào Concept `Log` của Knowledge Base `system`.

### 2.2. `LOGS_STREAM` Protocol
Khi một Admin Client gửi yêu cầu `LOGS_STREAM`, `ManagementManager` sẽ thực hiện cơ chế **Pub/Sub (Publish/Subscribe)**:
...
## 3. Lệnh Quản trị Tối cao (Management Command)

Đối với các tác vụ thay đổi cấu hình nóng (Hot-swapping config) hoặc khôi phục dữ liệu, Server cung cấp Message Type `MANAGEMENT_CMD (0x0D)`:
...
> [!TIP]
> **Khả năng Truy vấn Log thông minh**
