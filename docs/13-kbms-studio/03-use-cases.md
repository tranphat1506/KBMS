...
[KBMS](../00-glossary/01-glossary.md#kbms) Studio không chỉ là một trình soạn thảo, mà còn là một bộ công cụ giải quyết các bài toán tri thức chuyên sâu.

## 1. Kịch bản 1: Thiết kế Bản đồ Tri thức Hình học (KDL Design)

*   **Mục tiêu**: Định nghĩa các [Concept](../00-glossary/01-glossary.md#concept) như `Diem`, `DoanThang`, `TamGiac` và các `Rule` tính toán diện tích.
*   **Sơ đồ Hoạt động**:
...
*   **Quy trình chi tiết trong Studio**:
    1.  **Nhập liệu**: Người dùng sử dụng **[Monaco](../00-glossary/01-glossary.md#monaco) Editor** với tính năng Auto-complete để gõ nhanh `CREATE CONCEPT`.
    2.  **Kiểm tra**: [Parser](../00-glossary/01-glossary.md#parser) tại Server thực hiện biên dịch thời gian thực. Nếu sai, **[Error Squiggles](../00-glossary/01-glossary.md#error-squiggles)** sẽ xuất hiện ngay lập tức.
    3.  **Trực quan**: Khi cú pháp đúng, Designer sẽ vẽ lại sơ đồ [Concept](../00-glossary/01-glossary.md#concept) giúp người dùng kiểm tra các mối quan hệ (Properties/Constraints).
    4.  **Lưu trữ**: Sau khi hoàn thiện, tri thức được lưu vào tệp `.kbql` hoặc nạp trực tiếp vào RAM Server.

## 2. Kịch bản 2: Giải toán Tự động
...
*   **Quy trình chi tiết trong Studio**:
    ...
    2.  **Suy diễn**: Server chuyển yêu cầu cho `Reasoning Engine` để thực hiện thuật toán [F-Closure](../00-glossary/01-glossary.md#f-closure).
...
## 3. Kịch bản 3: Quản trị Hệ thống
...
*Hình 13.5: Các bước thao tác nạp dữ liệu lớn và bảo trì [B+ Tree](../00-glossary/01-glossary.md#b-tree).*

*   **Quy trình chi tiết trong Studio**:
    1.  **Nạp dữ liệu**: Dùng giao diện Import để đẩy các tệp CSV chứa hàng triệu Objects thông qua lệnh **[Bulk Insert](../00-glossary/01-glossary.md#bulk-insert)**.
    2.  **Giám sát**: Dashboard hiển thị thời gian thực mức độ chiếm dụng **RAM ([LRU](../00-glossary/01-glossary.md#lru) Cache)** và **Disk ([Page](../00-glossary/01-glossary.md#page) Storage)**.
    3.  **Bảo trì**: Thực hiện `MAINTENANCE REINDEX` trực tiếp từ Studio để tối ưu hóa cấu trúc cây [B+ Tree](../00-glossary/01-glossary.md#b-tree) sau khi nạp dữ liệu lớn.
    4.  **Kiểm tra**: Theo dõi **Activity Logs** để đảm bảo mọi giao dịch (Transaction) đều hoàn thành an toàn.

## 4. Kịch bản 4: Giám sát An ninh & Phiên làm việc
...
## 5. Kịch bản 5: Phân tích Nhật ký Chuyên sâu
...
*Hình 13.7: Luồng phân tích lỗi hệ thống thông qua [Log Analyzer](../00-glossary/01-glossary.md#log-analyzer).*

*   **Quy trình chi tiết trong Studio**:
    1.  **Lọc dữ liệu**: Sử dụng **[Log Analyzer](../00-glossary/01-glossary.md#log-analyzer)** để lọc các lỗi ở cấp độ `CRITICAL`.
    2.  **Truy vấn**: Sử dụng RegEx để tìm kiếm các cụm từ khóa liên quan đến thực thể (v.d: `TamGiac_01`).
    3.  **Đánh giá**: Studio hiển thị biểu đồ tần suất lỗi, giúp xác định lỗi xảy ra do [Rule](../00-glossary/01-glossary.md#rule) logic hay do xung đột [Page](../00-glossary/01-glossary.md#page) dữ liệu.
