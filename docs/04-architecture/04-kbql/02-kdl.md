...
## 4.4.1.2.2. Định nghĩa Khái niệm (Concept)
...
### 4.4.1.2.2.1. Tạo Concept (CREATE CONCEPT)
...
### 4.4.1.2.2.2. Chi tiết các khối nâng cao
Dưới đây là đặc tả các thành phần tùy chọn trong một Khối định nghĩa Tri thức:

• **ALIASES**: Định nghĩa các tên gọi khác cho Concept, giúp truy vấn linh hoạt hơn.
• **BASE_OBJECTS**: Liệt kê các đối tượng cơ sở cấu thành nên Concept này (thường dùng trong quan hệ `PART_OF`).
• **CONSTRAINTS**: Các ràng buộc logic (Boolean expressions) nội bộ mà mọi thực thể phải thỏa mãn (ví dụ: `age > 0`).
• **SAME_VARIABLES**: Khai báo các biến ở các thành phần khác nhau nhưng trỏ cùng một giá trị tri thức (Đồng nhất hóa biến).
• **CONSTRUCT_RELATIONS**: Tự động tạo lập các quan hệ thực thể khi dữ liệu được nạp vào hệ thống.
• **PROPERTIES**: Các đặc tính metadata bổ sung của Concept (như tác giả, phiên bản).
• **RULES / EQUATIONS**: Định nghĩa luật suy diễn và phương trình toán học ngay bên trong Concept thay vì tạo lệnh rời.

### 4.4.1.2.2.3. Chỉnh sửa Concept (ALTER CONCEPT)
...
Tối ưu hóa tốc độ truy vấn trên các biến cụ thể của Concept (sử dụng [B+ Tree](../../00-glossary/01-glossary.md#b-tree)).
