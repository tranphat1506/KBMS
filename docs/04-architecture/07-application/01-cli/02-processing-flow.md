### 4.7.1.2.1. Luồng Gửi lệnh

Khi người dùng nhấn Enter, [CLI](../../../00-glossary/01-glossary.md#cli) thực hiện các bước sau:

![Sơ đồ tuần tự mô tả luồng xử lý lệnh và phản hồi từ Server của CLI. | width=0.75](../../../assets/diagrams/cli_processing_flow.png)
*Hình 4.52: Sơ đồ tuần tự mô tả luồng xử lý lệnh và phản hồi từ Server của CLI.*

1.  **Input Buffering [I05]**: Tích lũy các dòng cho đến khi gặp dấu `;`.
...
Phần phức tạp nhất của CLI nằm ở `ResponseParser.cs`. Do kết quả từ Server có thể là một luồng dữ liệu ([Streaming](../../../00-glossary/01-glossary.md#streaming) ROWS), CLI phải xử lý từng gói tin:
...
1.  **[Header](../../../00-glossary/01-glossary.md#header) Rendering**: Ngay khi nhận `METADATA`, CLI tính toán độ rộng cột lớn nhất dựa trên tên cột và vẽ khung Header.
