...
Hệ quản trị cơ sở tri thức **[KBMS](../../00-glossary/01-glossary.md#kbms)** được xây dựng dựa trên kiến trúc phân lớp (Layered Architecture), cho phép tách biệt các tầng chức năng nhằm tối ưu hóa quá trình xử lý tri thức và quản trị dữ liệu. Kiến trúc này hỗ trợ việc chuyển đổi mô hình lý thuyết **[COKB](../../00-glossary/01-glossary.md#cokb)** thành một hệ thống thực thi ổn định, đảm bảo tính mở rộng và khả năng bảo trì mã nguồn trong dài hạn.
...
-   **Lớp Xử lý Server (Server Engine Layer)**: Là thành phần điều phối trung tâm của hệ thống. Tại đây, các câu lệnh ngôn ngữ **[KBQL](../../00-glossary/01-glossary.md#kbql)** được phân tích cú pháp để tạo thành Cây cú pháp trừu tượng (**AST**). Dựa trên AST, hệ thống điều hướng yêu cầu tới bộ máy suy diễn (**Inference Engine**) hoặc bộ phân tích truy vấn dữ liệu.
-   **Lớp Lưu trữ (Storage Layer)**: Đảm nhiệm việc lưu trữ và truy xuất dữ liệu từ các thiết bị lưu trữ thứ cấp. Sử dụng cấu trúc **Slotted Page** và chỉ mục **B+ Tree**, lớp này đảm bảo các thuộc tính **ACID** cho giao dịch và sử dụng nhật ký ghi trước (**WAL**) để phục hồi dữ liệu khi xảy ra sự cố.
...
3.  **Điều hướng**: Hệ thống kiểm tra loại lệnh trong AST. Nếu là lệnh suy diễn, thông tin sẽ được đưa vào mạng lưới **Rete**. Nếu là lệnh quản trị dữ liệu, hệ thống sẽ thực hiện truy xuất trực tiếp các trang dữ liệu (Pages) thông qua Buffer Pool.
...
| **Suy luận** | `KBMS.Reasoning.InferenceEngine`| Suy diễn tiến (Forward Chaining), Mạng Rete |
| **Lưu trữ** | `KBMS.Storage.V3` | Slotted Page, Cây B+ (B+ Tree), WAL |
