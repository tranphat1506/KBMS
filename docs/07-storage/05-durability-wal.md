...
- Mỗi khi một bản ghi nhật ký được tạo, `WalManagerV3` sẽ gán một [LSN](../00-glossary/01-glossary.md#lsn) tăng dần.
- Mỗi trang dữ liệu lưu trữ [LSN](../00-glossary/01-glossary.md#lsn) của thao tác thay đổi cuối cùng (tại Offset 4-7 trong [Header](../00-glossary/01-glossary.md#header)).
- **Quy tắc an toàn**: Một trang chỉ được phép ghi xuống đĩa nếu [LSN](../00-glossary/01-glossary.md#lsn) của trang đó nhỏ hơn hoặc bằng [LSN](../00-glossary/01-glossary.md#lsn) lớn nhất đã được ghi an toàn vào tệp `.wal`.

---

## 3. Quy trình Phục hồi sau sự cố

Khi [KBMS](../00-glossary/01-glossary.md#kbms) khởi động lại sau một sự cố ngắt điện (Crash), `WalManagerV3` thực hiện quy trình **Undo-based Recovery** để đưa hệ thống về trạng thái nhất quán gần nhất:
...
Cấu trúc một bản ghi nhật ký trong [KBMS](../00-glossary/01-glossary.md#kbms) V3 được thiết kế để chứa đủ thông tin cho cả việc **Undo** và **Redo**:
...
| :--- | :--- | :--- | :--- |
| **[TxnId](../00-glossary/01-glossary.md#txnid)** | [Guid](../00-glossary/01-glossary.md#guid) | 16 Bytes | Định danh duy nhất cho giao dịch. |
| **PageId** | Int32 | 4 Bytes | ID của trang bị thay đổi. |
| **BeforeLen** | Int32 | 4 Bytes | Độ dài của dữ liệu [Before-Image](../00-glossary/01-glossary.md#before-image). |
| **BeforeImage** | Blob | Biến thiên | Ảnh dữ liệu cũ (Dùng để **Undo**). |
| **AfterLen** | Int32 | 4 Bytes | Độ dài của dữ liệu [After-Image](../00-glossary/01-glossary.md#after-image). |
...
Dữ liệu thực tế được ghi vào tệp `.wal` khi một trang bị thay đổi được trích xuất từ engine (Ví dụ cho giao dịch thay đổi [Page](../00-glossary/01-glossary.md#page) 1):
...
- **PgId**: 4 bytes số nguyên (01 00 00 00 -> [Page](../00-glossary/01-glossary.md#page) 1).
- **BLen/ALen**: 4 bytes độ dài của ảnh dữ liệu trước/sau (0A 00 00 00 -> 10 bytes).
- **Committed**: 1 byte boolean (01 -> True).

Để ngăn tệp `.wal` phình to vô hạn, định kỳ [KBMS](../00-glossary/01-glossary.md#kbms) thực hiện quá trình [Checkpoint](../00-glossary/01-glossary.md#checkpoint):
1. **[Flush](../00-glossary/01-glossary.md#flush) Dirty Pages**: Ghi tất cả các trang bẩn từ [Buffer Pool](../00-glossary/01-glossary.md#buffer-pool) xuống tệp `.kdb`.
