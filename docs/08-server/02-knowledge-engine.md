...
> [!IMPORTANT]
> **Sự kết hợp hoàn hảo**
> Nhờ có `KnowledgeManager`, hệ thống có thể thực thi các câu lệnh phức tạp như `INFER` (đẩy sang Reasoning) và đồng thời ghi log `Audit Trail` của người dùng vào hệ thống `system` KB một cách đồng bộ.
