# 01. Khảo sát Hiện trạng & Mục tiêu (Survey & Objectives)
...
*   **CSDL Quan hệ (RDBMS):** Rất mạnh về lưu trữ đĩa (Persistence) và giao dịch (ACID) nhưng hoàn toàn thiếu khả năng suy diễn logic tự động. Việc xử lý tri thức phải thực hiện ở tầng ứng dụng, gây khó khăn cho việc bảo trì luật.
*   **Hệ thống Logic (Prolog):** Có khả năng suy diễn cực mạnh nhưng chủ yếu hoạt động trên bộ nhớ RAM, thiếu cơ chế quản lý dữ liệu nhị phân quy mô lớn trên đĩa cứng và khả năng quản lý đa người dùng thực thụ.
*   **Công cụ Ontologies (Protégé):** Hỗ trợ mô hình hóa tri thức tốt nhưng hiệu năng truy vấn và khả năng tích hợp vào các hệ thống phần mềm thương mại còn hạn chế.

### Bảng so sánh Tính năng Chi tiết

| Đặc tính | CSDL Quan hệ (MySQL) | Ngôn ngữ Logic (Prolog) | Công cụ Tri thức (Protégé) | **KBMS (Dự án này)** |
| :--- | :--- | :--- | :--- | :--- |
| **Kiểu dữ liệu** | Structured Tables | Logic Predicates | OWL / RDF Triples | **Knowledge Concepts** |
| **Suy diễn** | Không (Chỉ Join/Filter) | Rất mạnh (Backward) | Reasoner Plugin | **Mạnh (Forward F-Closure)** |
| **Lưu trữ đĩa** | B+ Tree, ACID | Chủ yếu trên RAM | Files (OWL/XML) | **B+ Tree nhị phân, WAL** |
| **Đa người dùng** | Rất tốt | Không | Hạn chế | **Tốt (Dựa trên Socket/Auth)** |
