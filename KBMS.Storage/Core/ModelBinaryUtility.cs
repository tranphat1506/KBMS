...
public static class ModelBinaryUtility
{
    ...
    public static byte[] SerializeConcept(Concept concept)
    {
        ...
        tuple.Fields.Add(SerializeList(concept.Indexes, WriteConceptIndex));
    ...
    public static Concept? DeserializeConcept(byte[] data)
    {
        ...
        try
        {
            ...
            return new Concept
            {
                ...
                Indexes = tuple.Fields.Count > 13 ? DeserializeList(tuple.Fields[13], ReadConceptIndex) : new List<ConceptIndex>()
            };
        }
        catch { return null; }
    }
    ...
    private static void WriteRule(BinaryWriter bw, Rule r)
    {
        ...
        bw.Write(r.ScopeConcepts.Count);
        foreach(var sc in r.ScopeConcepts) {
            bw.Write(sc.ConceptName ?? string.Empty);
            WriteNullableString(bw, sc.Alias);
            bw.Write(sc.Position);
        }

        bw.Write(r.JoinConditions.Count);
        foreach(var jc in r.JoinConditions) {
            bw.Write(jc.LeftField ?? string.Empty);
            bw.Write(jc.Operator ?? string.Empty);
            bw.Write(jc.RightField ?? string.Empty);
        }
    }
    ...
    private static Rule ReadRule(BinaryReader br)
    {
        ...
        try {
            r.Priority = br.ReadInt32();
            
            int scCount = br.ReadInt32();
            for(int i=0; i<scCount; i++) {
                r.ScopeConcepts.Add(new RuleScopeConcept {
                    ConceptName = br.ReadString(),
                    Alias = ReadNullableString(br),
                    Position = br.ReadInt32()
                });
            }

            int jcCount = br.ReadInt32();
            for(int i=0; i<jcCount; i++) {
                r.JoinConditions.Add(new RuleJoinCondition {
                    LeftField = br.ReadString(),
                    Operator = br.ReadString(),
                    RightField = br.ReadString()
                });
            }
    ...
    private static void WriteConceptIndex(BinaryWriter bw, ConceptIndex idx)
    {
        bw.Write(idx.Name ?? string.Empty);
        bw.Write(idx.Fields.Count);
        foreach(var f in idx.Fields) bw.Write(f ?? string.Empty);
    }
    
    private static ConceptIndex ReadConceptIndex(BinaryReader br)
    {
        var idx = new ConceptIndex {
            Name = br.ReadString(),
            Fields = new List<string>()
        };
        int count = br.ReadInt32();
        for(int k=0; k<count; k++) idx.Fields.Add(br.ReadString());
        return idx;
    }
