...
import { Protocol, MessageType, KbmsMessage } from './thingent-protocol';
...
export class KbmsTCPClient {
   ...
   connect(host: string = '127.0.0.1', port: number = 3307, user = 'admin', pass = 'admin'): Promise<boolean> {
      return new Promise((resolve) => {
         ...
         this.socket.connect(port, host, () => {
            ...
            this.sendToUI('thingent-status', 'connected');
         ...
         this.socket.on('close', () => {
            ...
            this.sendToUI('thingent-status', 'disconnected');
            this.socket = null;
         });
         ...
         this.socket.on('error', (err) => {
            ...
            this.sendToUI('thingent-status', 'error');
   ...
   getLspCompletions(code: string, line: number, column: number, kbName?: string): Promise<any> {
      return new Promise((resolve, reject) => {
         const requestId = `lsp_comp_${Date.now()}`;
         this.pendingRequestsMetadata.set(requestId, { isBackground: true });
         this.requestQueue.push({ 
            type: MessageType.LSP_AUTOCOMPLETE,
            query: JSON.stringify({ code, line, column, kb: kbName }),
            isBackground: true, 
            requestId, 
            resolve, 
            reject 
   ...
   getLspDiagnostics(code: string): Promise<any> {
      return new Promise((resolve, reject) => {
         const requestId = `lsp_diag_${Date.now()}`;
         this.pendingRequestsMetadata.set(requestId, { isBackground: true });
         this.requestQueue.push({ 
            type: MessageType.LSP_DIAGNOSTICS,
            query: JSON.stringify({ code }),
            isBackground: true, 
            requestId, 
            resolve, 
            reject 
   ...
   private handleServerMessage(msg: KbmsMessage) {
      ...
      if (msg.type === MessageType.ERROR) {
         ...
         this.sendToUI('thingent-stream', { type: 'error', text, markers, requestId: msg.requestId, isBackground });
      }
         ...
         try {
            ...
            if (metadata.error) {
               ...
               this.sendToUI('thingent-stream', { type: 'metadata', metadata, isBackground, requestId: msg.requestId });
            }
         } catch (e) {
         ...
         try {
            ...
            if (!Array.isArray(rowData) && rowData.error) {
               ...
               this.sendToUI('thingent-stream', { type: 'row', row: rowData, isBackground, requestId: msg.requestId });
            }
         } catch (e) {
         ...
         const isLsp = this.activeRequest?.type === MessageType.LSP_AUTOCOMPLETE || this.activeRequest?.type === MessageType.LSP_DIAGNOSTICS;
         
         if ((isMgmt || isLsp) && (msg.content.trim().startsWith('{') || msg.content.trim().startsWith('['))) {
            try {
            ...
            this.sendToUI('thingent-stream', { type: 'result', text, markers, isBackground, requestId: msg.requestId });
         }

         if (isLsp) {
            // Treat as FETCH_DONE so the promise resolves immediately
         ...
         try {
            ...
            this.sendToUI('thingent-stream', { type: 'log-signal', data: logData });
         } catch (e) {
         ...
         try {
            ...
            this.sendToUI('thingent-stream', { type: 'result', text, isBackground, requestId: msg.requestId });
         } catch { }
...
export const thingentClient = new KbmsTCPClient();
