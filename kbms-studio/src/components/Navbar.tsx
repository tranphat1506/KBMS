import { Play, Square, Save, FolderOpen, User } from 'lucide-react';
  ...
  return (
    <div className={`h-11 bg-[var(--bg-surface)] flex items-center px-4 justify-between border-b border-[var(--border-subtle)] shadow-[0_1px_3px_rgba(0,0,0,0.02)] z-20 font-sans select-none transition-colors duration-200 ${isMac ? 'pl-[90px]' : ''}`} style={{ WebkitAppRegion: 'drag' } as any}>
      <div className="flex items-center space-x-5" style={{ WebkitAppRegion: 'no-drag' } as any}>
        <div className="flex items-center space-x-1 select-none mr-2">
          <span className="font-semibold text-sm tracking-tight text-[var(--text-main)]">Thingent Studio</span>
        </div>
          ...
          <>
            <div className="flex items-center space-x-0.5 bg-[var(--bg-app)] p-0.5 rounded border border-[var(--border-subtle)] shadow-inner">
              <button 
                ...
                onClick={openTab} 
                className="flex items-center space-x-1 px-2.5 py-1 text-xs font-normal text-[var(--text-sub)] hover:bg-[var(--bg-surface)] hover:text-[var(--brand-primary)] hover:shadow-sm rounded transition-all cursor-pointer"
              >
              ...
              <button 
                 ...
                 disabled={!hasTabs}
                 className="flex items-center space-x-1 px-2.5 py-1 text-xs font-normal text-[var(--text-sub)] hover:bg-[var(--bg-surface)] hover:text-[var(--brand-primary)] hover:shadow-sm rounded transition-all disabled:opacity-50 disabled:hover:bg-transparent disabled:cursor-not-allowed cursor-pointer"
              >
            ...
            <div className="flex items-center space-x-2">
               <button 
                 ...
                 className={`flex items-center space-x-1.5 px-3 py-1 text-xs font-medium text-white rounded shadow-sm transition-all relative cursor-pointer ${
               ...
               <button 
                 ...
                 className={`flex items-center space-x-1.5 px-3 py-1 text-xs font-medium rounded shadow-sm border transition-all cursor-pointer ${
                     !isExecuting 
      ...
      <div className="flex items-center space-x-4 ml-4" style={{ WebkitAppRegion: 'no-drag' } as any}>
        {!isSystemView && (
          <select 
            value={selectedKb}
            onChange={(e) => useThingentStore.getState().changeKnowledgeBase(e.target.value)}
            disabled={status !== 'connected' || isExecuting} 
            className={`text-xs outline-none font-normal border rounded px-2 py-1 cursor-pointer transition-colors shadow-sm min-w-[150px] disabled:opacity-50 ${selectedKb === '' ? 'text-[var(--text-muted)] border-[var(--border-subtle)] bg-[var(--bg-app)]' : 'text-[var(--text-main)] border-[var(--brand-primary)]/40 bg-[var(--bg-surface)]'}`}
          >
            <option value="" disabled>Active Knowledge Base...</option>
            {useThingentStore.getState().serverMetadata.databases.map((db: string) => (
        ...
        <div className="flex items-center space-x-3 pl-4 border-l border-[var(--border-subtle)]">
           ...
           <div className="flex items-center space-x-2.5 px-2 py-1 bg-[var(--bg-app)] border border-[var(--border-subtle)] rounded shadow-sm group hover:border-[var(--brand-primary)]/30 transition-all cursor-default">
              ...
              <div className="flex flex-col">
                <span className="text-[2xs] font-bold text-[var(--text-main)] leading-none uppercase tracking-tighter">{lastCredentials?.user || 'Anonymous'}</span>
