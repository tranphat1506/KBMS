...
*Bảng 7.9: Phân rã kích thước trang vật lý sau khi mã hóa AES-256.*
...
Dưới đây là minh họa sự khác biệt giữa dữ liệu được lưu trữ "tĩnh" trên đĩa và dữ liệu "động" trong bộ nhớ RAM của KBMS.
...
```
Do được mã hóa AES-256, dữ liệu trên đĩa không có cấu trúc dễ đọc. Các trường như `PageId` hay `LSN` hoàn toàn bị xáo trộn, ngăn chặn mọi nỗ lực khai thác dữ liệu trái phép.
...
```
Cơ chế mã hóa trang (Page-level Encryption) đảm bảo dữ liệu chỉ tồn tại ở dạng tường minh trong vùng nhớ an toàn của KBMS Studio/Server và luôn ở trạng thái "rác vô nghĩa" khi nằm trên thiết bị lưu trữ vật lý.
