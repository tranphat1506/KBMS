...
*Bảng 7.3: Chiến lược lưu trữ nhị phân thống nhất trong KBMS.*

| Nhóm dữ liệu | Cấu trúc bộ nhớ | Cơ chế xử lý |
| :--- | :--- | :--- |
| **Thực thể (Object)** | Tuple (Offset-based) | Định dạng chuỗi UTF-8 tương thích schema |
| **Siêu dữ liệu (Meta)** | Tuple (Offset-based) | Đóng gói trực tiếp qua ModelBinaryUtility |
...
Tất cả các bản ghi trong KBMS (từ đối tượng người dùng đến định nghĩa tri thức) đều được lưu trữ trong một đơn vị `Tuple`. Cấu trúc này gồm 3 phần: **Field Count** (2B), **Offsets Array** ($N \times 2B$), và **Data Payload**.
...
| :--- | :--- | :--- | :--- |
| 0 | Id | GUID | Định danh duy nhất của đối tượng. |
...
| :--- | :--- | :--- | :--- |
| 0 | Id | GUID | Định danh duy nhất. |
| 1 | OwnerId | GUID | ID người sở hữu. |
...
| 11| Hierarchies | Blob | List\<Hierarchy\>. |
...
| :--- | :--- | :--- | :--- |
| 0 | Id | GUID | Định danh Concept. |
| 1 | KbId | GUID | ID KB chứa. |
...
Đối với các cấu trúc phức tạp như mô hình phân triển hoặc cây biểu thức, KBMS không sử dụng văn bản mà mã hóa đệ quy trực tiếp:
