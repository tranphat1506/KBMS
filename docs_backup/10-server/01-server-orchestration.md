...
Để tối ưu hóa thông lượng, KBMS sử dụng kiến trúc **Task-based Asynchronous Pattern ([TAP](../00-glossary/01-glossary.md#tap))** của .NET thay vì mô hình một-luồng-mỗi-khách-hàng (1-Thread-Per-Client) truyền thống.
...
Một đặc điểm đột phá của KBMS V3 là việc sử dụng chính mô hình tri thức [COKB](../00-glossary/01-glossary.md#cokb) để quản lý hệ thống. Một cơ sở tri thức đặc biệt tên là **`system`** (tệp `system.kdb`) được khởi tạo để lưu trữ:
